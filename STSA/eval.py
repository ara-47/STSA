...
from STSA.knapsack import knapsack
...
def select_keyshots(predicted_list, video_number_list,image_name_list,target_list,args):
    data_path = './STSA/datasets/datasets/'+str(args.dataset)+".h5"
    ...
    for i in range(len(video_single_list_sort)):
        ...
        fea_sequencelen = (len(video['feature'][:])//int(args.sequence))*int(args.sequence)
    ...
    for i in range(len(video_single_list_sort)):
        ...
        _, n_frames = true_summary_arr_20.shape
        eval_res = [eval_metrics(pred_summary, true_summary_1) for true_summary_1 in true_summary_arr_20]
        eval_res = np.average(eval_res, axis=0).tolist() if args.dataset == "TVSum" else np.max(eval_res, axis=0).tolist()
